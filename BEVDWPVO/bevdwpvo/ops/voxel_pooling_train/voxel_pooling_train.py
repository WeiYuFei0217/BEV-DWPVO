...
class VoxelPoolingTrain(Function):
    ...
    def forward(ctx, geom_xyz: torch.Tensor, input_features: torch.Tensor,
        ...
        """
        geom_xyz 的使用
            在这个体素池化操作中，geom_xyz 是用来确定每个体素在 BEV 特征图中的位置。具体来说：
            体素的 (x, y, z) 坐标被用来决定它在 BEV 图中的 (h, w) 位置。这是将3D点云数据映射到2D特征图的关键步骤。【没有depth监督的话，得到的depth深度就是单目估出来的，就不太准】
            在正向传播中，根据 geom_xyz 的值，每个体素的特征被汇总或分配到 BEV 特征图的相应位置。
            在反向传播中，geom_xyz 用于指导梯度从 BEV 特征图回流到原始体素特征。
        """
        ...
        output_features = input_features.new_zeros(batch_size, voxel_num[1],  # 这里为什么是[1],[0]
        ...
        voxel_pooling_train_ext.voxel_pooling_train_forward_wrapper(  # 调用的C++的库
            batch_size,
            num_points,
            num_channels,
            voxel_num[0],
